...
const BuyCredit = () => {
  ...
  const initPay = async (order) => {
    const options = {
        key: import.meta.env.VITE_RAZORPAY_KEY_ID || rzp_test_xrexzaq,
        amount: order.amount,
    ...
    try {
        ...
        const { data } = await axios.post(
            ...
            { planId },
            {
                headers: {
                    token,
                    "Content-Type": "application/json",  // ✅ This fixes your problem
                },
            }
        );
    ...
    >
      ...
      <div className='flex flex-wrap justify-center gap-6 text-left'>
        {plans.map((item, index) => (
          <div key={index} 
          className='bg-white drop-shadow-sm rounded-lg 
            ...
            <p className='mt-6'>
              ...
              <button onClick={() => paymentRazorpay(item.id)} 
                        className='w-full bg-gray-800 text-white mt-8 cursor-pointer text-sm 
                      rounded-md py-2.5 min-w-52  transition duration-300 
                      ease-in-out hover:scale-105 hover:bg-gray-700' >
